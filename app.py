...
# 群里用户点击 Telegram 菜单里的 /qp 但未带片名时，用这个临时记录下一条片名输入
PENDING_QP_INPUTS: Dict[str, Dict[str, Any]] = {}
PENDING_QP_INPUT_EXPIRE_SECONDS = int(os.getenv("PENDING_QP_INPUT_EXPIRE_SECONDS", "300"))
        ...
        try:
            ...
            for key, value in (store or {}).items():
                ...
                conn.execute(
                    ...
                    INSERT INTO users (
  				    tg_id, chat_id, username, first_name, last_name, display_name,
   				    quota_limit, success_count, remaining_quota,
   				    emby_username, emby_user_id, emby_bound_at,
				...
				ON CONFLICT(tg_id) DO UPDATE SET
				    ...
				    emby_bound_at=excluded.emby_bound_at,
...
def emby_get_public(path: str, params: Optional[Dict[str, Any]] = None) -> Any:
    if not emby_enabled():
        raise RuntimeError("未配置 EMBY_URL / EMBY_API_KEY")

    query = dict(params or {})
    query["api_key"] = EMBY_API_KEY
...
def build_emby_auth_header(user_id: str = "") -> str:
    return (
        f'Emby UserId="{user_id}", '
        f'Client="TelegramBot", '
        f'Device="TelegramBot", '
        f'DeviceId="telegram-bot-bind", '
        f'Version="1.0.0"'
    )


def emby_authenticate_user_by_name(username: str, password: str) -> Dict[str, Any]:
    if not EMBY_BASE:
        raise RuntimeError("未配置 EMBY_URL，无法绑定 Emby 账号")

    url = f"{EMBY_BASE}/Users/AuthenticateByName"
    headers = {
        "Accept": "application/json",
        "Content-Type": "application/json",
        "X-Emby-Authorization": build_emby_auth_header(""),
    }
    payload = {
        "Username": username,
        "Pw": password,
    ...
    emby_user_id = str(user.get("Id") or "").strip()
    emby_username = str(user.get("Name") or username).strip()
...
def is_user_emby_bound(user_id: int) -> bool:
    record = get_user_record(user_id)
    return bool(str(record.get("emby_user_id") or "").strip())


def clear_other_accounts_with_same_emby_user(emby_user_id: str, exclude_tg_id: Optional[int] = None) -> None:
    emby_user_id = str(emby_user_id or "").strip()
    if not emby_user_id:
        return
        ...
        try:
            if exclude_tg_id:
                conn.execute(
                    """
                    UPDATE users
                    SET emby_username = '', emby_user_id = '', emby_bound_at = '', updated_at = ?
                    WHERE emby_user_id = ? AND tg_id <> ?
                    """,
                    (now_str, emby_user_id, _safe_int(exclude_tg_id, 0)),
                )
            else:
                conn.execute(
                    """
...
def get_user_record_by_emby_user_id(emby_user_id: str) -> Dict[str, Any]:
    emby_user_id = str(emby_user_id or "").strip()
    if not emby_user_id:
        return {}
...
def bind_user_emby_account(user_id: int, emby_username: str, emby_user_id: str) -> Dict[str, Any]:
    emby_user_id = str(emby_user_id or "").strip()
    ...
    record = get_user_record(user_id)
    current_emby_user_id = str(record.get("emby_user_id") or "").strip()

   # 当前 TG 已经绑定过了：不允许直接覆盖
    if current_emby_user_id:
        if current_emby_user_id == emby_user_id:
            raise ValueError(f"你当前已经绑定该 Emby 账号：{emby_username}")
        raise ValueError("你当前已经绑定了其他 Emby 账号。")

    # 该 Emby 账号已被其他 TG 绑定：禁止抢绑
    bound_record = get_user_record_by_emby_user_id(emby_user_id)
    if bound_record and _safe_int(bound_record.get("tg_id"), 0) != _safe_int(user_id, 0):
        bound_text = format_bound_tg_account_text(bound_record)
        raise ValueError(f"该 Emby 账号已与其他 Telegram 账号绑定：{bound_text}")

    record["emby_username"] = emby_username
    record["emby_user_id"] = emby_user_id
    record["emby_bound_at"] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    record["updated_at"] = record["emby_bound_at"]
    return _upsert_user_record_to_db(record)


def clear_user_emby_binding(user_id: int) -> Dict[str, Any]:
    record = get_user_record(user_id)
    record["emby_username"] = ""
    record["emby_user_id"] = ""
    record["emby_bound_at"] = ""
    record["updated_at"] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    return _upsert_user_record_to_db(record)


def get_tg_display_name(record: Dict[str, Any]) -> str:
    ...
    tg_id = _safe_int(record.get("tg_id"), 0)
    return str(tg_id or "未知用户")

def format_bound_tg_account_text(record: Dict[str, Any]) -> str:
    if not isinstance(record, dict):
        return "未知账号"

    display_name = get_tg_display_name(record)
    username = str(record.get("username") or "").strip().lstrip("@")
    tg_id = _safe_int(record.get("tg_id"), 0)
    ...
    if tg_id:
        return f"{display_name} (TG ID: {tg_id})"
    return display_name


def format_tg_mention_markdown(record: Dict[str, Any]) -> str:
    name_text = escape_markdown_v2(get_tg_display_name(record))
    username = str(record.get("username") or "").strip().lstrip("@")
    tg_id = _safe_int(record.get("tg_id"), 0)

    if username:
        return f"[{name_text}](https://t.me/{username})"
    if tg_id:
        return f"[{name_text}](tg://user?id={tg_id})"
    return name_text


def format_ticks_hms(ticks: Any) -> str:
    ticks_int = _safe_int(ticks, 0)
    if ticks_int <= 0:
        return ""

    total_seconds = ticks_int // 10_000_000
...
def format_emby_session_media_title(item: Dict[str, Any]) -> str:
    if not isinstance(item, dict) or not item:
        return ""

    item_type = str(item.get("Type") or item.get("MediaType") or "").lower()
    name = str(item.get("Name") or "").strip()

    if item_type == "episode":
        series_name = str(item.get("SeriesName") or "").strip() or name or "未知剧集"
        season_number = item.get("ParentIndexNumber")
        episode_number = item.get("IndexNumber")
        episode_tag = format_season_episode_tag(season_number, episode_number)
        episode_name = name if name and name != series_name else ""
    ...
    return f"{name} ({year})" if name and year else (name or "未知内容")


def get_session_status_text(session_obj: Dict[str, Any]) -> str:
    play_state = session_obj.get("PlayState") or {}
    now_playing = session_obj.get("NowPlayingItem") or session_obj.get("NowViewingItem") or {}
...
def clean_client_name(client: str) -> str:
    client = str(client or "").strip()
    if not client:
        return ""

    # 去掉括号里的系统信息，比如：
    # AfuseKt/(Linux;Android Release)Player-TV -> AfuseKt/Player-TV
...
def get_session_client_text(session_obj: Dict[str, Any]) -> str:
    client = clean_client_name(session_obj.get("Client") or "")
    device_name = str(session_obj.get("DeviceName") or "").strip()

    if client and device_name and device_name != client:
        return f"{client}｜{device_name}"
    if client:
        return client
    if device_name:
        return device_name
...
def get_emby_online_sessions() -> List[Dict[str, Any]]:
    data = emby_get_public("/Sessions")
...
def get_bf_session_rank(session_obj: Dict[str, Any]) -> int:
    status = str(session_obj.get("status") or "")
    if "播放中" in status:
        return 0
    if "暂停中" in status:
        return 1
...
def get_bf_user_sort_key(user_entry: Dict[str, Any]) -> Any:
    best_rank = 9
    fallback_name = str(user_entry.get("fallback_name") or "")
    sessions_list = user_entry.get("sessions") or []
    for sess in sessions_list:
        best_rank = min(best_rank, _safe_int(sess.get("rank"), 9))
    return (best_rank, fallback_name.lower())


def build_bf_message(sessions: List[Dict[str, Any]]) -> str:
    grouped: Dict[str, Dict[str, Any]] = {}

    for session_obj in sessions:
        user_id = str(session_obj.get("UserId") or "").strip()
        session_id = str(session_obj.get("Id") or "").strip()
        if not user_id or not session_id:
            continue

        status_text = get_session_status_text(session_obj)
        if "播放中" not in status_text and "暂停中" not in status_text:
            continue

        bound_record = get_user_record_by_emby_user_id(user_id)
        if bound_record:
            user_key = f"tg:{_safe_int(bound_record.get('tg_id'), 0)}"
            display_user = format_tg_mention_markdown(bound_record)
            fallback_name = str(
                bound_record.get("emby_username")
                or session_obj.get("UserName")
                or get_tg_display_name(bound_record)
                or "未知用户"
            ).strip()
        else:
            user_key = f"emby:{user_id}"
            emby_name = str(session_obj.get("UserName") or "未知用户").strip()
            display_user = escape_markdown_v2(f"{emby_name}（未绑定TG）")
            fallback_name = emby_name
        ...
        now_playing = session_obj.get("NowPlayingItem") or session_obj.get("NowViewingItem") or {}
        play_state = session_obj.get("PlayState") or {}
        media_title = format_emby_session_media_title(now_playing)
        position_text = format_ticks_hms(play_state.get("PositionTicks"))
        runtime_text = format_ticks_hms(now_playing.get("RunTimeTicks"))
        progress_text = ""
        if position_text and runtime_text:
            progress_text = f"{position_text} / {runtime_text}"
        elif position_text:
            progress_text = position_text

        user_entry["sessions"].append(
            {
                "status": status_text,
                "rank": get_bf_session_rank({"status": status_text}),
                "client": get_session_client_text(session_obj),
                "media_title": media_title or "未知内容",
    ...
    if not grouped:
        return (
            "🎬 *Emby 在线播报*\n"
            "👥 *在线人数：* 0\n"
            f"🕒 *时间：* {escape_markdown_v2(now_text)}\n\n"
            f"{DIVIDER}\n"
            "🧾 *当前没有检测到正在播放或暂停中的用户*"
        )

    sorted_users = sorted(grouped.values(), key=get_bf_user_sort_key)

    lines = [
        "🎬 *Emby 在线播报*",
        f"👥 *在线人数：* {len(sorted_users)}",
        f"🕒 *时间：* {escape_markdown_v2(now_text)}",
    ]

    for user_entry in sorted_users:
        sessions_list = sorted(
            user_entry.get("sessions") or [],
            key=lambda x: (_safe_int(x.get("rank"), 9), str(x.get("media_title") or "").lower()),
        )
        if not sessions_list:
            continue

        primary = sessions_list[0]
        lines.extend(
            [
                "",
                DIVIDER,
                f"👤 {user_entry['display_user']}",
                f"┣ *状态：* {escape_markdown_v2(primary['status'])}",
                f"┣ *内容：* {escape_markdown_v2(primary['media_title'])}",
            ]
        )
        if primary.get("progress"):
            lines.append(f"┣ *进度：* {escape_markdown_v2(primary['progress'])}")
        lines.append(f"┗ *客户端：* {escape_markdown_v2(primary['client'])}")

    lines.extend(["", DIVIDER])
    return "\n".join(lines).rstrip()

def handle_bf(message: Dict[str, Any]) -> None:
    chat_id = int(message["chat"]["id"])

    # 改为：只要绑定了 Emby 账号就能用
    if not require_user_emby_bound(message):
        return

    if not emby_enabled():
        send_message(chat_id, "❌ Emby 未配置，无法查询在线用户。")
        return

    try:
        sessions = get_emby_online_sessions()
        text = build_bf_message(sessions)
        send_message(chat_id, text, parse_mode="MarkdownV2", disable_web_page_preview=True)
    except Exception:
        logging.exception("查询 Emby 在线会话失败")
        send_message(chat_id, "❌ 查询 Emby 在线会话失败，请稍后重试。")


def require_user_emby_bound(message: Dict[str, Any]) -> bool:
    chat_id = int((message.get("chat") or {}).get("id") or 0)
    user_id = _safe_int((message.get("from") or {}).get("id"), 0)

    if not user_id:
        send_message(chat_id, "❌ 无法识别你的 Telegram 账号。")
        return False

    if is_user_emby_bound(user_id):
        return True
...
def handle_bind(message: Dict[str, Any], args: str) -> None:
    chat_id = int(message["chat"]["id"])

    # 强制只允许私聊
    if is_group_chat(message):
        send_message(chat_id, "❌ /bd 请私聊机器人绑定 Emby 账号。")
        return

    parts = args.split(maxsplit=1)
    if len(parts) != 2:
        send_message(chat_id, "用法：/bd 账号 密码")
        return

    emby_username = parts[0].strip()
    emby_password = parts[1]

    if not emby_username or not emby_password.strip():
        send_message(chat_id, "❌ 账号或密码不能为空。\n用法：/bd 账号 密码")
        return

    tg_user_id = _safe_int((message.get("from") or {}).get("id"), 0)
    current_record = get_user_record(tg_user_id)
    current_emby_user_id = str(current_record.get("emby_user_id") or "").strip()
    current_emby_username = str(current_record.get("emby_username") or "").strip()

    # 已经绑定过：不允许再次绑定，必须先解绑
    if current_emby_user_id:
        current_name = current_emby_username or "未知账号"
        send_message(
            chat_id,
            "❌ 你当前已经绑定了 Emby 账号，不能再次绑定。\n\n"
            f"当前绑定账号：{current_name}\n"
        )
        return

    try:
        auth_result = emby_authenticate_user_by_name(emby_username, emby_password)
    except Exception:
        logging.exception("Emby 账号绑定失败")
        send_message(chat_id, "❌ Emby 校验失败，请稍后重试。")
        return

    if not auth_result:
        send_message(chat_id, "❌ 账号或密码错误，绑定失败。")
        return

    # 校验该 Emby 账号是否已被其他 TG 绑定
    bound_record = get_user_record_by_emby_user_id(auth_result["emby_user_id"])
    if bound_record and _safe_int(bound_record.get("tg_id"), 0) != tg_user_id:
        bound_text = format_bound_tg_account_text(bound_record)
        send_message(
            chat_id,
            "❌ 该 Emby 账号已与其他 Telegram 账号绑定。\n\n"
            f"当前绑定 TG：{bound_text}\n"
            "请先由原绑定账号解绑后再绑定。"
        )
        return

    try:
        bind_user_emby_account(
            tg_user_id,
            auth_result["emby_username"],
            auth_result["emby_user_id"],
        )
    except ValueError as e:
        send_message(chat_id, f"❌ {e}")
        return
    except Exception:
        logging.exception("写入 Emby 绑定信息失败")
        send_message(chat_id, "❌ 绑定失败，保存数据时出错，请稍后重试。")
        return

    send_message(
        chat_id,
        "✅ Emby 账号绑定成功\n\n"
        f"当前绑定账号：{auth_result['emby_username']}\n"
        "现在你可以使用Bot啦。"
    )

def handle_unbind(message: Dict[str, Any], args: str) -> None:
    chat_id = int(message["chat"]["id"])

    if is_group_chat(message):
        send_message(chat_id, "❌ /jb 只能私聊使用。")
        return
    ...
    current_emby_user_id = str(record.get("emby_user_id") or "").strip()
    current_emby_username = str(record.get("emby_username") or "").strip()

    if not current_emby_user_id:
        send_message(
            chat_id,
            "❌ 你当前还没有绑定 Emby 账号。\n\n"
            "请先发送：\n"
    ...
    password = (args or "").strip()
    if not password:
        send_message(
            chat_id,
            "⚠️ 解绑需要输入当前 Emby 密码二次确认。\n\n"
            "用法：/jb 你的Emby密码"
        )
        return

    if not current_emby_username:
        send_message(
            chat_id,
            "❌ 当前绑定信息缺少 Emby 用户名，无法校验密码。\n"
            "请先重新绑定后再解绑。"
        )
        return

    try:
        auth_result = emby_authenticate_user_by_name(current_emby_username, password)
    except Exception:
        logging.exception("解绑时校验 Emby 密码失败")
        send_message(chat_id, "❌ Emby 校验失败，请稍后重试。")
        return

    if not auth_result:
        send_message(chat_id, "❌ 密码错误，解绑失败。")
        return

    authed_emby_user_id = str(auth_result.get("emby_user_id") or "").strip()
    if authed_emby_user_id != current_emby_user_id:
        send_message(chat_id, "❌ 密码校验通过，但账号与当前绑定账号不一致，解绑失败。")
        return

    clear_user_emby_binding(user_id)
...
def build_emby_exists_text(item: Dict[str, Any]) -> str:
...
def notify_user_emby_exists(chat_id: int, item: Dict[str, Any]) -> None:
    send_message(chat_id, build_emby_exists_text(item), parse_mode="MarkdownV2")


def edit_user_choice_message_result(
    callback_message: Dict[str, Any],
    fallback_chat_id: int,
    text: str,
    parse_mode: Optional[str] = "MarkdownV2",
) -> bool:
    """把用户原来的版本/季选择消息改成最终结果提示。"""
    chat_id = _safe_int((callback_message.get("chat") or {}).get("id"), 0) or _safe_int(fallback_chat_id, 0)
    message_id = _safe_int(callback_message.get("message_id"), 0)
    ...
    try:
        ...
        logging.exception("编辑用户选择消息为结果提示失败")
        return False


def delete_user_choice_message(callback_message: Dict[str, Any], fallback_chat_id: int) -> None:
    chat_id = _safe_int((callback_message.get("chat") or {}).get("id"), 0) or _safe_int(fallback_chat_id, 0)
    message_id = _safe_int(callback_message.get("message_id"), 0)
...
def build_channel_new_item_text(
    ...
    if media_type == "剧集":
        ...
        # 多季剧：优先显示当前季首播日期
        release_date = (
            (stats or {}).get("season_air_date")
            or item.get("_tmdb_release_date")
            or ""
        )
    else:
        release_date = item.get("_tmdb_release_date") or ""
    ...
    if release_date:
        lines.append(f"📅 *上映日期：* {escape_markdown_v2(release_date)}")
...
def enrich_item_with_tmdb(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 已有 TMDB ID：继续去 TMDB 拉详情，补 rating / title / year / 上映日期
    if tmdb_id:
        ...
        try:
            ...
            if tmdb_detail:
                ...
                item["_tmdb_release_date"] = (tmdb_detail.get("release_date") or "").strip()
        except Exception:
...
def get_tmdb_detail_for_notify(media_type: str, tmdb_id: Any) -> Optional[Dict[str, Any]]:
    ...
    if media_type == "movie":
        ...
        release_date = (detail.get("first_air_date") or "").strip()
        year = release_date[:4]
...
def get_tmdb_season_detail_for_notify(tv_tmdb_id: Any, season_number: int) -> Optional[Dict[str, Any]]:
    ...
    return {
        "season_number": season_number,
        "season_name": detail.get("name") or "",
...
def delete_message(chat_id: int, message_id: int) -> None:
    ...
    try:
        ...
        logging.warning(
            "删除消息失败，已忽略：chat_id=%s message_id=%s err=%s",
            chat_id,
            message_id,
            e,
        )
...
def _pending_qp_key(chat_id: int, user_id: int) -> str:
    return f"{chat_id}:{user_id}"


def cleanup_pending_qp_inputs() -> None:
    now_ts = time.time()
    expired_keys = []
    for key, value in list(PENDING_QP_INPUTS.items()):
        created_at = _safe_int(value.get("created_at_ts"), 0)
        if created_at <= 0 or now_ts - created_at > PENDING_QP_INPUT_EXPIRE_SECONDS:
            expired_keys.append(key)

    for key in expired_keys:
        PENDING_QP_INPUTS.pop(key, None)


def set_pending_qp_input(
    chat_id: int,
    user_id: int,
    prompt_message_id: int,
    command_message_id: int = 0,
) -> None:
    cleanup_pending_qp_inputs()
    PENDING_QP_INPUTS[_pending_qp_key(chat_id, user_id)] = {
        "chat_id": _safe_int(chat_id, 0),
        "user_id": _safe_int(user_id, 0),
        "prompt_message_id": _safe_int(prompt_message_id, 0),
        "command_message_id": _safe_int(command_message_id, 0),
        "created_at_ts": int(time.time()),
    }


def pop_pending_qp_input(chat_id: int, user_id: int) -> Dict[str, Any]:
    cleanup_pending_qp_inputs()
    return PENDING_QP_INPUTS.pop(_pending_qp_key(chat_id, user_id), {})


def build_qp_input_prompt_text() -> str:
    return (
        "🎬 请输入要搜索的影片名\n\n"
        "直接发送影片名即可，例如：沙丘"
    )
...
def build_requester_mention_markdown(requester_user_id: int, requester_name: str = "") -> str:
    """构造群内回执顶部的用户提醒文本。

    有真实 Telegram username 时，直接输出纯文本 @username，尽量触发群内 @ 提醒。
    没有 username 时，退回到 tg://user?id= 的可点击昵称。
    返回值已经按 MarkdownV2 处理，可以直接拼进消息。
    """
    requester_user_id = _safe_int(requester_user_id, 0)
    requester_name = str(requester_name or "").strip()

    record: Dict[str, Any] = {}
    if requester_user_id > 0:
        try:
            record = get_user_record(requester_user_id) or {}
        except Exception:
            record = {}

    username = str((record or {}).get("username") or "").strip().lstrip("@")
    if not username and requester_name.startswith("@"):
        candidate = requester_name[1:].strip()
        if re.fullmatch(r"[A-Za-z0-9_]{5,32}", candidate):
            username = candidate

    if username:
        # 不做 Markdown 链接，直接发纯文本 @username，更容易触发 Telegram 群提醒。
        return escape_markdown_v2("@" + username)

    display_name = ""
    if record:
        display_name = get_tg_display_name(record)
    if not display_name or display_name == "未知用户":
        display_name = requester_name or (str(requester_user_id) if requester_user_id > 0 else "未知用户")

    if requester_user_id > 0:
        return f"[{escape_markdown_v2(display_name)}](tg://user?id={requester_user_id})"
...
def build_user_result_text(
    ...
    requester_mention = str(detail.get("_requester_mention_markdown") or "").strip()
    requester_line = f"*求片人：* {requester_mention}\n\n" if requester_mention else ""
...
def build_user_select_keyboard(
    ...
    for item in results:
        ...
        callback_data = f"s|{media_type}|{tmdb_id}|{requester_user_id}|{reply_chat_id}|{_safe_int(command_message_id, 0)}"
...
def build_tv_season_select_keyboard(
    ...
    for item in valid_seasons:
        ...
        callback_data = f"ss|{tmdb_id}|{season_number}|{requester_user_id}|{reply_chat_id}|{_safe_int(command_message_id, 0)}"
...
def submit_user_request(
    ...
    if existing:
        exists_text = build_emby_exists_text(existing)

        # 资源已存在时，也直接把原来的“版本/季选择”消息改成提示，
        # 不再额外发送一条消息，避免群里刷屏。
        if not edit_user_choice_message_result(
            callback_message,
            requester_chat_id,
            exists_text,
            parse_mode="MarkdownV2",
            ...
            delete_user_choice_message(callback_message, requester_chat_id)
            send_message(requester_chat_id, exists_text, parse_mode="MarkdownV2")

        # 群里用 /qp 或菜单输入片名发起的请求，查到已存在后也删除用户那条片名/命令消息。
        if _safe_int(command_message_id, 0) > 0:
            delete_message(requester_chat_id, _safe_int(command_message_id, 0))
        return
    ...
    title = get_display_name(detail, media_type, season_number=season_number)
    selected_text = f"✅ 你已选择：*{escape_markdown_v2(title)}*\n已发送给管理员审核。"

    # 直接把原来的“版本/季选择”消息改成提交成功提示，不再新发一条消息
    if not edit_user_choice_message_result(
        callback_message,
        requester_chat_id,
        selected_text,
        ...
        logging.warning("编辑用户选择消息失败，改为删除原选择消息后发送结果")
        remove_review_keyboard(callback_message)
        delete_user_choice_message(callback_message, requester_chat_id)
        send_message(
            requester_chat_id,
    ...
    # 群里用 /qp 发起的命令，提交审核成功后自动删除用户那条 /qp 消息
    if _safe_int(command_message_id, 0) > 0:
        delete_message(requester_chat_id, _safe_int(command_message_id, 0))
...
def send_search_choices_to_user(
    ...
    # /qp 不带片名时，机器人会先发一条“请输入影片名”的提示；
    # 用户输入片名后，直接把这条提示编辑成选择版本的消息，不再额外新发一条。
    if _safe_int(selection_message_id, 0) > 0:
        try:
            edit_message_text(
                chat_id=chat_id,
                message_id=_safe_int(selection_message_id, 0),
                text=text,
                parse_mode="MarkdownV2",
                reply_markup=reply_markup,
                disable_web_page_preview=True,
            ...
            logging.exception("编辑 /qp 片名输入提示为选择列表失败，改为发送新消息")
...
def handle_search(message: Dict[str, Any], query: str, command_message_id: int = 0, selection_message_id: int = 0) -> None:
    if not require_user_emby_bound(message):
        return
    ...
    if not results:
        no_result_text = f"❌ 没有找到和 *{escape_markdown_v2(query)}* 相关的电影或剧集，请换个关键词试试。"
        if _safe_int(selection_message_id, 0) > 0:
            try:
                edit_message_text(
                ...
                logging.exception("编辑 /qp 无结果提示失败，改为发送新消息")
...
def handle_callback_query(callback: Dict[str, Any]) -> None:
    ...
    if action_code in ("s", "ss") and not is_user_emby_bound(from_user_id):
       answer_callback(callback_id, "请先绑定 Emby 账号", show_alert=True)
       if callback_chat_id:
           send_message(
               int(callback_chat_id),
               "❌ 你还没有绑定 Emby 账号，请先私聊发送：/bd Emby账号 Emby密码"
           )
       return
    ...
    if action_code == "s":
        try:
            ...
            command_message_id = _safe_int(parts[5], 0) if len(parts) >= 6 else 0
    ...
    try:
        ...
        # 群里发送批准/拒绝回执时，在卡片顶部带上 @用户名，方便用户看到。
        if requester_chat_id < 0:
            detail["_requester_mention_markdown"] = build_requester_mention_markdown(
                requester_user_id,
                str(req.get("requester_name") or ""),
            )
...
def handle_message(message: Dict[str, Any]) -> None:
    ...
    if command == "/start":
        record = get_user_record(from_user_id)

        if is_user_emby_bound(from_user_id):
            send_message(
                chat_id,
                "欢迎使用 Emby 求片 Bot\n\n"
                f"你当前已绑定 Emby 账号：{record.get('emby_username') or '未知'}\n\n"
                "用法：\n"
                "1. 私聊可直接发送片名，例如：沙丘\n"
                "2. 也可以发送 /qp 片名\n"
                "3. 发送 /id 查看你自己的 UID\n"
    ...
    if command == "/help":
        send_message(
            ...
            "/start - 查看欢迎信息\n"
            "/help - 查看帮助\n"
            "/bd 账号 密码 - 绑定 Emby 账号（仅私聊）\n"
            "/bf - 查看 Emby 在线用户（仅管理员）\n"
            "/id - 查看你的 UID\n"
            "/cx - 查看剩余求片次数\n"
            "/qp 片名 - 求片\n"
        )
    ...
    if command == "/bd":
        handle_bind(message, args)
        return
        
    if command == "/jb":
        handle_unbind(message, args)
        return
    ...
    if command == "/qp":
        if not args:
            ...
            prompt_result = send_message_with_result(
                chat_id,
                build_qp_input_prompt_text(),
                disable_web_page_preview=True,
            )
            prompt_message_id = _safe_int((prompt_result or {}).get("message_id"), 0)

            if prompt_message_id > 0:
                set_pending_qp_input(
                    chat_id=chat_id,
                    user_id=from_user_id,
                    prompt_message_id=prompt_message_id,
                    command_message_id=_safe_int(message.get("message_id"), 0) if in_group else 0,
                )

            # 群里点快捷命令产生的 /qp 消息尽量删掉，避免刷屏；失败不影响后续流程。
            if in_group:
                delete_message(chat_id, _safe_int(message.get("message_id"), 0))
            return

        # 群里 /qp 命令提交成功后删除这条命令消息；私聊不删除
        command_message_id = _safe_int(message.get("message_id"), 0) if in_group else 0
        handle_search(message, args, command_message_id=command_message_id)
        return

    if command:
        return

    pending_qp = pop_pending_qp_input(chat_id, from_user_id)
    if pending_qp:
        # 用户点击 /qp 快捷命令后输入的影片名：
        # 1. 直接编辑 Bot 那条提示为选择版本列表；
        # 2. 成功提交审核后再删除用户这条片名消息。
        input_message_id = _safe_int(message.get("message_id"), 0) if in_group else 0
        prompt_message_id = _safe_int(pending_qp.get("prompt_message_id"), 0)
        ...
        if prompt_message_id > 0:
            try:
                edit_message_text(
                    ...
                    text=f"🔎 正在搜索：{escape_markdown_v2(text)}",
                    parse_mode="MarkdownV2",
                    reply_markup={"inline_keyboard": []},
                    disable_web_page_preview=True,
                )
            except Exception:
                logging.exception("编辑 /qp 搜索中提示失败，继续执行搜索")

        handle_search(
            message,
            text,
            command_message_id=input_message_id,
            selection_message_id=prompt_message_id,
        )
        return
...
def ensure_users_binding_columns(conn: sqlite3.Connection) -> None:
    cols = {str(row["name"]) for row in conn.execute("PRAGMA table_info(users)").fetchall()}

    if "emby_username" not in cols:
        conn.execute("ALTER TABLE users ADD COLUMN emby_username TEXT DEFAULT ''")
    if "emby_user_id" not in cols:
        conn.execute("ALTER TABLE users ADD COLUMN emby_user_id TEXT DEFAULT ''")
    if "emby_bound_at" not in cols:
        conn.execute("ALTER TABLE users ADD COLUMN emby_bound_at TEXT DEFAULT ''")
...
def init_sqlite_db() -> None:
    ...
    with DB_LOCK:
        ...
        try:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS users (
  			     ...
  			     display_name TEXT,
   			     ...
   			     emby_username TEXT DEFAULT '',
 			     emby_user_id TEXT DEFAULT '',
			     emby_bound_at TEXT DEFAULT '',
			     updated_at TEXT,
			     last_seen_at TEXT
			 );
            ...
            ensure_users_binding_columns(conn)
            conn.commit()
        finally:
            conn.close()
...
def _build_user_record(existing: Optional[Dict[str, Any]], tg_id: int, chat_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    # 新增：Emby 绑定信息
    record['emby_username'] = (existing.get('emby_username') or '').strip()
    record['emby_user_id'] = str(existing.get('emby_user_id') or '').strip()
    record['emby_bound_at'] = str(existing.get('emby_bound_at') or '').strip()
        ...
        try:
            conn.execute(
                ...
                (
                    ...
                    (record.get('emby_username') or '').strip(),
                    str(record.get('emby_user_id') or '').strip(),
                    str(record.get('emby_bound_at') or '').strip(),
...
def upsert_user_profile(user: Dict[str, Any], chat_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    first_name = user.get('first_name') or ''
    last_name = user.get('last_name') or ''
    display_name = (f"{first_name} {last_name}".strip()) or username or str(tg_id)
...
def compute_tv_notify_stats(job: Dict[str, Any], item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if season_detail and season_detail.get("total_count", 0) > 0:
        ...
        pending_numbers = sorted(unaired_numbers)

        is_ongoing = len(pending_numbers) > 0
        ...
        stats.update({
            ...
            'pending_text': format_episode_list(pending_numbers),
            'season_air_date': (season_detail.get('air_date') or '').strip(),
        })
        return stats
...
def build_tv_stats_lines(stats: Dict[str, Any]) -> List[str]:
    ...
    total_count = _safe_int(stats.get('total_count'), 0)

    if stats.get('is_ongoing'):
        lines.extend([
            f"📦 *总集数：* {escape_markdown_v2(total_count)} 集",
...
def notify_channel_new_item(
    ...
    if not force_skip_cache and should_skip_recent_notify(item):
        logging.info("同一波入库重复通知，已跳过：%s", title)
        return
    ...
    if image_url:
        logging.info(
            "准备发送入库图片通知：title=%s image_url=%s",
            title,
            mask_url_secret(image_url),
        )
        ...
        try:
            ...
            logging.info("入库图片通知发送成功：title=%s", title)
            return
        except Exception:
            logging.exception(
                "入库 sendPhoto 失败，回退到纯文本通知：title=%s image_url=%s",
                title,
                mask_url_secret(image_url),
            )
    else:
        logging.warning("入库通知无图片地址，直接发送纯文本：%s", title)
    ...
    logging.info("入库纯文本通知发送成功：%s", title)
